...
void DCT(ld matrix[8][8],int secret)
{
    ...
    if(we==0){
        cout<<"The DC component and secret pixel are \n"<<dct[0][0]<<' '<<secret<<'\n';
    }
    ll abcd=dct[0][0];
    abcd-=(abcd&15);
    abcd|=secret>>4;
    ...
    dct[0][0]=abcd;
    if(we==0){
        cout<<"The DC component after hiding secret message is \n"<<dct[0][0]<<'\n';
        cout<<"The Pixel values after IDCT are :\n";
    }
    iDCT(dct);
    f(i,8){
        f(j,8){
    ...
    if(we==0){
        cout<<"The Pixels integral values are \n";
        f(i,8){
            f(j,8){
                matrix[i][j]=(int)dct[i][j];
                if(we==0){
                    cout<<matrix[i][j]<<' ';
                }
            }
        ...
        if(we==0){
            cout<<"The DC component after making the pixels to integers \n"<<dct[0][0]<<'\n';
        }
    }
