...
int main(){
    ...
    f(i,unhid.cols){
        f(j,unhid.rows){
            f(k,3){
                int a[4];
                a[0]=image.at<Vec3b>(j*2,i*2).val[k]&1;
                a[1]=image.at<Vec3b>(j*2+1,i*2).val[k]&1;
                a[2]=image.at<Vec3b>(j*2,i*2+1).val[k]&1;
                a[3]=image.at<Vec3b>(j*2+1,i*2+1).val[k]&1;
                unhid.at<Vec3b>(j, i).val[k]=(a[3]<<7)|(a[2]<<6)|(a[1]<<5)|(a[0]<<4);
            }
        }
    }
